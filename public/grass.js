...
function wavingGrass() {
	...
	*/
	function grassObj(x, y, seg1, seg2, maxAngle, grassWidth) {
		...
		this.gradient = getGradient(Math.random() * 50 + 50, 100 * Math.random() + 10);
		this.currentAngle;
	...
	function makeGrass(numOfGrass, width, height, hVariation, grassWidth) {
		...
		for(; i < numOfGrass; i++) {
			...
			x = width * Math.random();
			y = 200 - hf * Math.random();
			
			seg1 = y / 3 + y * hVariation * Math.random() * 0.1;
			seg2 = (y / 3 * 2) + y * hVariation * Math.random() * 0.1;
			
			grass.push(new grassObj(x, y, seg1, seg2, 15 * Math.random() + 75, 10));
		}

		return grass;
	}//change 500 for amount of grass
	grass = makeGrass(500, w, h * 0.4, 0.3, grassWidth);
    ...
    function updateGrass() {
        gh = 100/100;
    	grass = makeGrass(500, w, h * gh, 0.3, grassWidth);
        num.innerHTML = 500;    
        num2.innerHTML = 100;
